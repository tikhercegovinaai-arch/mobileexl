...
export class ExcelExportService {
    ...
    static async exportToExcel(
        ...
        if (format === 'pdf') {
            return ExcelExportService._exportToPdf(fields, filename);
        }
    ...
    private static async _exportToPdf(fields: ValidationField[], filename?: string): Promise<string> {
        const rowsHtml = fields.map(f => `
            <tr>
                <td style="padding: 8px; border: 1px solid #ddd;">${f.category || 'General'}</td>
                <td style="padding: 8px; border: 1px solid #ddd;">${f.label}</td>
                <td style="padding: 8px; border: 1px solid #ddd;">${f.value}</td>
                <td style="padding: 8px; border: 1px solid #ddd; color: #${confidenceColor(f.confidence).substring(2)}; font-weight: bold;">
        ...
        const html = `
            <html>
                <body style="font-family: sans-serif; padding: 20px;">
                    <h1 style="color: #1F2937;">Extraction Report</h1>
                    ...
                    <div style="margin-top: 30px; border-top: 1px solid #eee; padding-top: 10px; font-size: 12px; color: #9CA3AF;">
                        Generated by Exelent Scanner
                    </div>
                </body>
            </html>
        `;
        ...
        // If filename is provided, we might want to move it to a specific name in the cache
        // but for now, uri from printToFileAsync is enough.
        return uri;
    }
}
